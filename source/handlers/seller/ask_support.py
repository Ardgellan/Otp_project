...
from source.utils.states.seller_states import AskSupport
...
async def ask_user_for_question_to_support(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        text=localizer.get_user_localized_text(
            ...
            text_localization=localizer.message.ask_support_question_message,
        ),
...
async def forward_question_to_admins(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        text=localizer.get_user_localized_text(
            ...
            text_localization=localizer.message.support_question_sent_by_user_message,
        ),
        reply_markup=await inline.insert_button_back_to_main_menu(
        ...
        try:
            await forwarded_message.reply(
                text=localizer.get_user_localized_text(
                    ...
                    text_localization=localizer.message.admin_notification_about_new_support_question_message,
                ).format(
